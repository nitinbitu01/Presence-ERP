...
export function ProfileForm({
  depts, progs, pDept, pProg, pNcc, pSem, pRoll,
  profileBusy, profileIncomplete,
  setPDept, setPProg, setPNcc, setPSem, setPRoll, onSave,
}: Props) {
  const filteredProgs = progs.filter(
    (p) => !p.department_id || p.department_id === pDept,
  );
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <select
            id="pf-dept"
            value={pDept}
            onChange={(e) => { setPDept(e.target.value); setPProg(""); }}
      ...
      <div className="mt-3 pt-3 border-t border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 p-2 rounded-md border border-input bg-muted/20 text-xs">
          {[
            ...
            <label key={sub.code} className="flex items-center gap-2 p-1.5 rounded hover:bg-muted/40 cursor-pointer">
