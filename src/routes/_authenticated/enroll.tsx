...
import {
  saveEnrollment, requestEnrollmentChallenge, getEnrolledPhoto,
} from "@/lib/attendance.functions";
import { generateVoicePassphrase } from "@/lib/voice-verification.server";
import {
  hasEnrollment, withdrawBiometric, getMyProfile,
  updateMyProfile, listDepartments, listPrograms, getMyRoles,
} from "@/lib/admin.functions";
import {
  startWebauthnRegistration, finishWebauthnRegistration,
  listMyWebauthnDevices, removeWebauthnDevice,
} from "@/lib/webauthn.functions";
import {
  computeDeviceFingerprint, detectFacesCount,
  captureLivenessFrameSequence, loadFaceApi, estimateHeadAngles,
} from "@/lib/face-api-loader";
import {
  assessFrameQuality, averageEmbeddings, isEmbeddingEligible,
  MIN_PASSING_FRAMES, QUALITY_REASON_MESSAGES,
} from "@/lib/face-quality";
import {
  verifyLivenessSignals,
  type LivenessChallenge, type LivenessAction,
...
const FALLBACK_DEPTS: Dept[] = [
  { id: "bd77422e-39be-4796-b990-634f040eba6b", code: "SASET", name: "School of Advanced Sciences, Engineering and Technology (SASET)" },
  { id: "f2efc950-5aa2-4fa5-a96b-f441496c4637", code: "SITAICS", name: "School of Information Technology, AI & Cyber Security (SITAICS)" },
  { id: "3eb09141-b36b-4bb9-b5c7-520138390b81", code: "SISDSS", name: "School of Internal Security, Defence & Strategic Studies (SISDSS)" },
  { id: "9f3ac950-250c-4930-96d1-8f581c355928", code: "SISSP", name: "School of Internal Security and Strategic Policy (SISSP)" },
  { id: "1b1c68c9-190a-4fe9-b064-60a89c26830a", code: "SPES", name: "School of Physical Education and Sports (SPES)" },
];

const FALLBACK_PROGS: Prog[] = [
  { id: "10000000-0000-0000-0000-000000000001", department_id: "f2efc950-5aa2-4fa5-a96b-f441496c4637", code: "BTECH-CS", name: "B.Tech Computer Science & Engineering" },
  { id: "10000000-0000-0000-0000-000000000002", department_id: "f2efc950-5aa2-4fa5-a96b-f441496c4637", code: "BTECH-CY", name: "B.Tech Cyber Security" },
  { id: "10000000-0000-0000-0000-000000000003", department_id: "f2efc950-5aa2-4fa5-a96b-f441496c4637", code: "MTECH-AI", name: "M.Tech Artificial Intelligence" },
  { id: "10000000-0000-0000-0000-000000000004", department_id: "f2efc950-5aa2-4fa5-a96b-f441496c4637", code: "MSC-DS", name: "M.Sc Data Science & Analytics" },
  { id: "10000000-0000-0000-0000-000000000005", department_id: "bd77422e-39be-4796-b990-634f040eba6b", code: "BTECH-SASET", name: "B.Tech Advanced Engineering" },
  { id: "10000000-0000-0000-0000-000000000006", department_id: "3eb09141-b36b-4bb9-b5c7-520138390b81", code: "MA-SS", name: "M.A. Strategic Studies" },
  { id: "10000000-0000-0000-0000-000000000007", department_id: "9f3ac950-250c-4930-96d1-8f581c355928", code: "MA-SP", name: "M.A. Strategic Policy" },
  { id: "10000000-0000-0000-0000-000000000008", department_id: "1b1c68c9-190a-4fe9-b064-60a89c26830a", code: "BPED", name: "Bachelor of Physical Education" },
];
...
function cleanErrorMessage(e: unknown): string {
  ...
  if (typeof e === "string") { msg = e; }
    ...
    if (msg === "[object Object]") {
      try { msg = JSON.stringify(e); } catch { msg = "An error occurred."; }
    }
  }
  if (msg.includes("Unauthorized") || msg.includes("No authorization token") ||
      msg.includes("not signed in") || msg.includes("Invalid token")) {
    return "You are not signed in. Please sign in to continue.";
  }
  const isHtml = /<!doctype|<html|<head|this page didn't load|something went wrong|500 internal/i.test(msg);
...
class EnrollErrorBoundary extends Component<
...
function EnrollPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const [
          enrollResult, rolesResult, profileResult,
          deptsResult, progsResult, devicesResult,
        ...
        // Departments
        const fetchedDepts = deptsResult.status === "fulfilled"
          ? (deptsResult.value as Dept[])
          : [];
        setDepts(fetchedDepts.length > 0 ? fetchedDepts : FALLBACK_DEPTS);

        // Programs
        const fetchedProgs = progsResult.status === "fulfilled"
          ? (progsResult.value as Prog[])
          : [];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!subscribed) { stream.getTracks().forEach((t) => t.stop()); return; }
        activeStream = stream;
        if (videoRef.current) {
  ...
  const onSaveProfile = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const onCaptureAndVerify = useCallback(async () => {
    ...
    try {
      ...
      if (!check.passed) {
        const REASONS: Record<string, string> = {
          turn_right_not_detected: "Right head turn not detected. Please turn head right when prompted.",
          turn_left_not_detected: "Left head turn not detected. Please turn head left when prompted.",
      ...
      for (let i = 0; i < seq.frameEmbeddings.length; i++) {
        ...
        if (landmarks && bbox) {
          ...
          if (!q.passed) { q.reasons.forEach((r) => allQualityReasons.add(r)); continue; }
        }
    ...
    state.consent, state.model.status, dispatch,
    reqEnrollChallenge, capturePhotoSnapshot,
  ]);
  ...
  const onRemoveDevice = useCallback(async (id: string) => {
    dispatch({ type: "DEVICE_BUSY", value: true });
    dispatch({ type: "DEVICE_ERROR", error: null });
    try {
      const headers = await getAuthHeaders();
  ...
  const onSpeakPassphrase = useCallback(() => {
    const SR = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
    if (!SR) { dispatch({ type: "VOICE_ERROR", error: "Speech recognition not supported." }); return; }
    dispatch({ type: "VOICE_LISTENING_START" });
    const rec = new SR();
    rec.lang = "en-US"; rec.interimResults = false; rec.maxAlternatives = 1;
    rec.onresult = (event: any) => {
      dispatch({ type: "VOICE_LISTENING_DONE", transcript: event.results?.[0]?.[0]?.transcript ?? "" });
    };
    rec.onerror = () => dispatch({ type: "VOICE_ERROR", error: "Could not capture speech. Try again." });
    rec.onend = () => { if (state.voiceListening) dispatch({ type: "VOICE_LISTENING_DONE", transcript: "" }); };
    rec.start();
  ...
  const profileIncomplete =
    !state.isAdminOrTeacher && (!state.profile || !state.profile.department_id || !state.profile.roll_no);

  const isAuthError = state.error && (
    state.error.includes("signed in") || state.error.includes("Unauthorized")
  );
  ...
  if (isAuthError) {
    return (
      <div className="mx-auto max-w-md px-6 py-16 text-center">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-amber-500/10 text-3xl">🔑</div>
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Your camera image is processed <strong>only in this browser</strong>. We never
        store raw images or video. What leaves your device is a 128-dimension numeric
        descriptor, encrypted with AES-GCM on the server.
      </p>
      ...
      {!state.isAdminOrTeacher && (
        <ProfileForm
          ...
          depts={depts}
          progs={progs}
          pDept={pDept} pProg={pProg} pNcc={pNcc} pSem={pSem} pRoll={pRoll}
          profileBusy={state.profileBusy}
          profileIncomplete={profileIncomplete}
          setPDept={setPDept} setPProg={setPProg} setPNcc={setPNcc}
          setPSem={setPSem} setPRoll={setPRoll}
        ...
        <div className="mt-6 rounded-lg border border-border bg-card p-6 text-center shadow-md">
          ...
          <div className="mb-2 flex flex-wrap justify-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-blue-500/15 px-3 py-1 text-xs font-semibold text-blue-700 dark:text-blue-300">
              📷 {state.pendingCapture.qualityPassedFrames}/{state.pendingCapture.qualityTotalFrames} frames passed
            </span>
          </div>
      ...
      <p aria-live="polite" className="mt-2 text-xs text-muted-foreground">{statusMsg}</p>
      ...
      {state.stage === "capture" && (
        <div className="mt-6 space-y-3 rounded-lg border border-border bg-card p-4 text-sm text-card-foreground">
          ...
          <p className="text-muted-foreground">
            Under GDPR Art. 9, India's DPDP Act, and BIPA, face descriptors are
            special-category biometric data. By continuing you grant explicit, revocable
            consent for Presence to store your encrypted descriptor for up to 365 days,
            solely to verify your identity at attendance.
          </p>
          ...
          <label className="flex items-start gap-2">
            <input
              ...
              className="mt-0.5"
            />
            <span>Allow non-biometric fallback (OTP + teacher justification) if biometric fails.</span>
          </label>
            ...
            <label className="flex items-start gap-2">
              ...
              <span>
                ...
                <span className="block text-xs text-muted-foreground mt-0.5">
                  Borderline face matches can be resolved by speaking a passphrase instead
                  of waiting for teacher approval.
                </span>
              </span>
            </label>
            {state.voiceEnroll && (
              <div className="mt-3 space-y-2">
                {!state.speechSupported ? (
                  ...
                  <>
                    ...
                    >
                      {state.voiceListening ? "Listening… speak the passphrase" : "Speak passphrase"}
                    </button>
                    ...
                    {state.voiceError && (
                      <p role="alert" className="text-xs text-destructive">{state.voiceError}</p>
                    )}
                  </>
                )}
              </div>
      ...
      {state.stage === "capture" && (
        <div className="mt-6 flex flex-wrap gap-3">
          <button
            disabled={!state.consent || state.busy || profileIncomplete || state.model.status !== "ready"}
          ...
          <button onClick={onSignOut} className="rounded-md border border-input px-4 py-2 text-sm hover:bg-accent">
            Sign out
          </button>
        </div>
      )}
      ...
      <p className="mt-6 text-xs text-muted-foreground">
        Read the full{" "}
        <Link to="/privacy" className="text-primary underline">privacy &amp; biometric data policy</Link>.
      </p>
      ...
      <div className="mt-8 border-t border-border pt-6">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Registering Face ID / Touch ID / Windows Hello adds hardware-backed check-in on top of face verification.
        </p>
        {!state.isMounted ? (
          ...
          <>
              ...
              <ul className="mt-3 space-y-1 text-sm" aria-label="Registered devices">
                {state.devices.map((d) => (
                  <li key={d.id} className="flex items-center justify-between gap-2">
                    <span>{d.device_label || "Unnamed device"} — {new Date(d.created_at).toLocaleDateString()}</span>
                    <button
            ...
            {state.deviceError && <p role="alert" className="mt-2 text-xs text-destructive">{state.deviceError}</p>}
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <label htmlFor="bottom-device-label" className="sr-only">Device name</label>
              <input
