...
import {
  isWebNfcSupported,
  readNfcTagUid,
} from "@/lib/adapters/web-nfc-checkin.adapter";
...
function cleanErrorMessage(e: unknown): string {
  ...
  if (typeof e === "string") {
    ...
    const candidate =
      obj.message ?? obj.error ?? obj.statusText ?? String(e);
...
class AttendErrorBoundary extends Component<
  { children: ReactNode },
  { error: Error | null }
> {
  state = { error: null };
...
function AttendPage() {
  ...
  const [showVoicePassphraseInput, setShowVoicePassphraseInput] =
    useState(false);
  ...
  const getFreshPosition = useCallback(async (): Promise<
    Omit<CachedPosition, "cachedAt">
  > => {
  ...
  const onCheckIn = useCallback(async () => {
    ...
    if (attemptCountRef.current > MAX_CHECKIN_ATTEMPTS) {
      setError(
        `Maximum ${MAX_CHECKIN_ATTEMPTS} attempts reached. Please refresh the page.`,
      );
      return;
    }
    ...
    try {
      ...
      const seq = await captureLivenessFrameSequence(
        videoRef.current,
        (curr, total) =>
          setStatus(`Scanning face: frame ${curr}/${total}…`),
        challenge.action,
      );
  ...
  const onVoiceVerify = useCallback(async () => {
    ...
    rec.onresult = (event: any) => {
      ...
      setVoiceStatus(
        "Heard your speech. Enter the passphrase you were asked to speak:",
      );
    };
    rec.onerror = () => {
      setVoiceListening(false);
  ...
  if (
    error &&
    (error.includes("signed in") || error.includes("Unauthorized"))
  ) {
    return (
      <div className="mx-auto max-w-md px-6 py-16 text-center">
      ...
      <div className="mx-auto max-w-xl px-6 py-12">
        <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-6 text-red-900 dark:text-red-200">
          ...
          <p className="mt-2 text-sm">
            {modelState.reason ||
              "The biometric verification engine failed to load."}
          </p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      {attemptCountRef.current >= MAX_CHECKIN_ATTEMPTS - 1 && (
        ...
        >
          ⚠ {MAX_CHECKIN_ATTEMPTS - attemptCountRef.current} attempt(s)
          remaining before page reload is required.
        </div>
      )}
      ...
      <div className="mt-4">
        ...
        <input
          ...
          value={sessionOtp}
          onChange={(e) =>
            setSessionOtp(e.target.value.replace(/\D/g, ""))
          }
          placeholder="123456"
      ...
      {spatialAnchor && (
        <SpatialAnchorRadar signals={spatialAnchor} className="mt-4" />
      )}
      ...
      <div className="mt-4 rounded-lg border border-blue-500/30 bg-blue-500/5 p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Accessible alternative to the camera flow. Requires NFC tag bound
          to your account and Android Chrome.
        </p>
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <button
            ...
            aria-label={
              nfcBusy ? "Waiting for NFC tap…" : "Check in via NFC"
            }
      ...
      {/* Error display */}
      {error && (
        <div role="alert" className="mt-4 rounded-lg border border-destructive/30 bg-destructive/10 p-4 space-y-2">
...
function VoiceVerificationPanel({
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Your face match was borderline. Speak your enrollment passphrase to
        auto-resolve without waiting for teacher approval.
      </p>
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <button
        ...
        >
          {voiceListening
            ? "Listening… speak your passphrase"
            : "Verify by voice"}
        </button>
        {voiceTranscript && (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          If your camera, browser permissions, or GPS is unavailable, submit
          a fallback request for teacher review.
        </p>
