...
export function computeTrustScore(
  ...
  const livenessAchieved = similarity != null ? Math.max(0, Math.min(1, (similarity - 0.5) / 0.5)) : 0;
  components.push({
    label: 'Face liveness match',
    weight: WEIGHTS.liveness,
    achieved: livenessAchieved,
    detail: similarity != null ? `${(similarity * 100).toFixed(1)}% embedding similarity` : 'unavailable',
  });
  ...
  const total = Math.round(
    components.reduce((sum, c) => sum + c.weight * c.achieved, 0),
  );
