...
import type { HardwareCheckinAdapter, HardwareCheckinPayload, HardwareCheckinResult, HardwareType } from "@/lib/hardware-checkin-adapter.server";
...
export async function readNfcTagUid(timeoutMs = 10_000): Promise<string> {
    if (!isWebNfcSupported()) {
        throw new Error("Web NFC is not supported in this browser. Use Android Chrome with NFC enabled.");
    }
    ...
    return new Promise<string>((resolve, reject) => {
        ...
        reader.scan().catch((err: unknown) => {
            clearTimeout(timer);
            reject(new Error(`NFC scan failed to start: ${err instanceof Error ? err.message : "unknown error"}`));
        });
    });
}
