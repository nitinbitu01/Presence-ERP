...
export function validatePassword(
  ...
  if (config.preventCommonPatterns) {
    const commonPatterns = [
      ...
      { regex: /(012|123|234|345|456|567|678|789|890)/, message: "Password contains sequential numbers" },
      { regex: /(abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|jkl|klm|lmn|mno|nop|opq|pqr|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz)/i, message: "Password contains sequential letters" },
  ...
  try {
    ...
    if (allHistory && allHistory.length > PASSWORD_CONFIG.historyCount) {
      const idsToDelete = allHistory
        .slice(PASSWORD_CONFIG.historyCount)
        .map((h: any) => h.id);
