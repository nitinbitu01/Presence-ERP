...
export default defineTool({
  ...
  description: 'Explain why a specific attendance record was marked present/absent/review, citing the underlying gate signals and trust score breakdown.',
  inputSchema: {
  ...
  handler: async (input: any, ctx: any) => {
    ...
    let query = supabase
      ...
      .select('id, session_id, student_id, decision, similarity, gate_reasons, trust_score, trust_breakdown, reason_code, created_at')
    ...
    if (!rows?.length) return { content: [{ type: 'text' as const, text: 'No attendance records found matching your query.' }] };
