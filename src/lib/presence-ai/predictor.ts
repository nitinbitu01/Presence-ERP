...
export function compute(
  ...
  const bestCaseRate = (total + remaining) > 0
    ? (attended + remaining) / (total + remaining)
    : 1;

  const worstCaseRate = (total + remaining) > 0
    ? attended / (total + remaining)
    : 0;

  const needed = Math.max(
    0,
    Math.ceil(THRESHOLD * (total + remaining) - attended),
  );
