...
export async function loadFaceApi(): Promise<any> {
  ...
  try {
    ...
    const clean = isHtml
      ...
      : raw;
    throw new Error(
      clean.startsWith("VERIFICATION_UNAVAILABLE")
        ? clean
        : `VERIFICATION_UNAVAILABLE: ${clean}`,
    );
  }
}
...
export function estimateHeadAngles(positions: { x: number; y: number }[]): { yaw: number; pitch: number } {
...
export async function captureLivenessFrameSequence(
  ...
  const probeEmbedding = Array.from({ length: dim }, (_, i) =>
  ...
  return { probeEmbedding, frameEmbeddings, livenessSignals, frameLandmarks, frameBboxes, frameActions };
}
