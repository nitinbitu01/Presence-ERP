...
import { verifyLivenessSignals, verifyFrameIdentityConsistency, cosineSimilarity, haversineMeters } from "../src/lib/attendance-crypto.server";

test.describe("Security Attack Harness — Playwright E2E & Server Function Assertion Suite", () => {
  const classroomLat = 23.153421;
  const classroomLng = 72.886547;
  const classroomRadiusM = 50;

  test("Attack 1: Photo / Static-Image Spoof is caught by Gate 8 (short-frame & long-frame)", async () => {
  ...
  test("Attack 2: Video Replay — frame-swap inconsistency is caught", async () => {
  ...
  test("Attack 3: Wrong-Face Impersonation — similarity thresholds enforced", async () => {
  ...
  test("Attack 5: GPS Spoofing — Geofence radius and synthetic accuracy", async () => {
