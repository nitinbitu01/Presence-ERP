...
async function main() {
  ...
  const passA = assertGate("Short-frame photo spoof (5 frames)", resultA, {
    ...
    expectedReasonCode:
      resultA.reasonCode?.includes("attestation") ? "device_attestation_missing" : "liveness_static_photo_detected",
  ...
  const passB = assertGate("Long-frame photo spoof (10 frames)", resultB, {
    expectedReasonCode:
      resultB.reasonCode?.includes("attestation") ? "device_attestation_missing" : "liveness_static_photo_detected",
    expectedDecision: "rejected",
  });
  ...
  const passC = assertGate("Hand-held photo spoof (low areaVar)", resultC, {
    expectedReasonCode:
      resultC.reasonCode?.includes("attestation") ? "device_attestation_missing" : "liveness_static_photo_detected",
    expectedDecision: "rejected",
  });
  ...
  const realFaceSignals = Array.from({ length: 10 }, (_, i) => ({
    ear: 0.30 + Math.sin(i * 0.8) * 0.06,         // natural EAR variation (breathing affects eye openness)
  ...
  if (!photoGate?.reason?.includes("static_photo")) {
    ...
    console.log(`  ${"\x1b[31m"}✗\x1b[0m Real face rejected as static photo — thresholds too tight, loosen them`);
  }
  ...
  if (total < 3) {
    console.log("→ Tighten verifyLivenessSignals thresholds in attendance-crypto.server.ts and re-run");
  }
}
