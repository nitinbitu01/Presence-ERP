...
async function main() {
  ...
  const resultA = await callSubmitAttendance(sessionA, makePayload(
    sessionA.userId,
    DEVICE_FP.studentA,
  ...
  const resultB = await callSubmitAttendance(sessionB, makePayload(
    sessionB.userId,
    DEVICE_FP.studentB,
  ...
  const reasons = resultB.gateReasons as Record<string, { ok?: boolean; reason?: string }> | undefined;
  ...
  if (resultB.decision === "rejected") {
    ok(`Accomplice rejected! Caught by: ${caughtBy.join(", ") || resultB.reasonCode || "unknown gate"}`);
  } else {
  ...
  const resultB2 = await callSubmitAttendance(sessionB, makePayload(
    sessionB.userId,
    DEVICE_FP.studentB,
  ...
  if (resultB2.decision === "rejected") {
    ok(`Accomplice in room rejected! Caught by: ${caughtBy2.join(", ") || resultB2.reasonCode || "unknown"}`);
  } else {
