...
async function main() {
  ...
  const students = [
    config.testStudents[0],
    config.testStudents[1],
    config.testStudents[2],
  ];

  const sessions: Array<{ session: Awaited<ReturnType<typeof signIn>>; student: typeof students[0] }> = [];
