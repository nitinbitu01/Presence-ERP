...
async function main() {
  ...
  if (subjects.length === 0) { console.error(`No subject folders found in ${DATASET_DIR}`); process.exit(1); }

  console.log(`Found ${subjects.length} subject(s): ${subjects.join(", ")}\n`);

  const results = [];
  let totalProbes = 0, autoAccepted = 0, sentToReview = 0, rejected = 0, noFaceDetected = 0;
  ...
  for (const subject of subjects) {
    ...
    if (!referenceFile) { console.warn(`  [${subject}] No reference.jpg found — skipping.`); continue; }

    const referenceDescriptor = await extractDescriptorFromFile(join(subjectDir, referenceFile));
    if (!referenceDescriptor) { console.warn(`  [${subject}] No face in reference — skipping.`); continue; }
    ...
    for (const probeFile of probeFiles) {
      ...
      let decision;
      if (similarity >= THRESHOLD_MATCH) { decision = "auto_accepted"; autoAccepted++; }
      else if (similarity >= THRESHOLD_REVIEW) { decision = "sent_to_review"; sentToReview++; }
      else { decision = "rejected"; rejected++; }
  ...
  const byCondition = {};
  for (const r of results) { if (!byCondition[r.condition]) byCondition[r.condition] = []; byCondition[r.condition].push(r); }
  ...
  const csv = ["subject,condition,similarity,decision", ...results.map((r) => `${r.subject},${r.condition},${r.similarity ?? ""},${r.decision}`)].join("\n");
  writeFileSync(join(OUT_DIR, "results.csv"), csv);
  writeFileSync(join(OUT_DIR, "results.json"), JSON.stringify({
    generatedAt: new Date().toISOString(), datasetDir: DATASET_DIR,
    thresholds: { match: THRESHOLD_MATCH, review: THRESHOLD_REVIEW },
    subjects: subjects.length, totalProbes, autoAccepted, sentToReview, rejected, noFaceDetected,
    falseRejectEstimate: falseRejects.length, results,
  }, null, 2));
