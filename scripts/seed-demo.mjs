...
const SUPABASE_URL = process.env.VITE_SUPABASE_URL || process.env.SUPABASE_URL || "https://omewkcnzhgptspgljrnc.supabase.co";
...
async function run() {
  ...
  const departments = [
    ...
    { name: 'School of Information Technology, Artificial Intelligence and Cyber Security', code: 'SITAICS' },
  ...
  try {
    for (const dept of departments) {
      ...
      if (course) {
        ...
        console.log(`// Note: students, teachers, courses, and attendance history would be fully seeded here if Supabase auth constraints allow.`);
      }
    }
