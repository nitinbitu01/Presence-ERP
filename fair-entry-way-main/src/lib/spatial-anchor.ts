...
function singleGpsFix(timeoutMs: number, retries = 2): Promise<GeolocationPosition> {
...
function analyseMockLocationRisk(samples: GpsSample[]): MockLocationAnalysis {
  ...
  if (samples.length >= 3) {
    const latRange =
      Math.max(...samples.map((s) => s.lat)) - Math.min(...samples.map((s) => s.lat));
    const lngRange =
      Math.max(...samples.map((s) => s.lng)) - Math.min(...samples.map((s) => s.lng));

    if (latRange < STATIC_COORDINATE_THRESHOLD_DEG && lngRange < STATIC_COORDINATE_THRESHOLD_DEG) {
  ...
  const score = Math.min(
    ...
    (Object.entries(signals) as [keyof MockLocationSignals, boolean][]).reduce(
      (sum, [key, triggered]) => (triggered ? sum + MOCK_SIGNAL_WEIGHTS[key] : sum),
      0,
    ),
  );
  ...
  const triggeredSignalNames = (Object.entries(signals) as [keyof MockLocationSignals, boolean][])
    .filter(([, v]) => v)
    .map(([k]) => k);
    ...
    return new Promise<MotionPresenceResult>((resolve) => {
      ...
      const handler = (event: DeviceMotionEvent) => {
        ...
        resolve({
          ...
          acceleration: acc !== null ? { x: acc.x ?? 0, y: acc.y ?? 0, z: acc.z ?? 0 } : null,
          rotationRate:
            rot !== null
...
export async function captureSpatialAnchor(
  ...
  const effectiveNonce =
    nonce && nonce.trim().length > 0 ? nonce : `client-nonce-${crypto.randomUUID()}`;
  const effectiveHmacSecret =
    hmacSecret && hmacSecret.trim().length > 0
      ? hmacSecret
      : `client-secret-${crypto.randomUUID()}`;

  const [{ samples: gpsSamples, firstError }, network, deviceFingerprint, motion] =
    await Promise.all([
      collectGpsSamples(sampleCount, sampleIntervalMs, gpsTimeoutMs),
      captureNetworkInfo(),
      captureDeviceFingerprint(),
      captureMotion
        ? captureMotionPresence()
        : Promise.resolve<MotionPresenceResult>({
            apiAvailable: false,
            motionDetected: false,
            acceleration: null,
            rotationRate: null,
...
export async function validateSpatialPayload(
  ...
  if (geofence.radiusM < MIN_GEOFENCE_RADIUS_M || geofence.radiusM > MAX_GEOFENCE_RADIUS_M) {
    throw new Error(
