  ...
  .handler(async ({ data, context }) => {
      ...
      for (const row of existingRows) {
        ...
        try {
          ...
          if (sim >= THRESHOLD_MATCH && !demoActive) {
            ...
            const ciphertextForQueue = `\\x${Array.from(await encryptEmbedding(data.embedding))
    ...
    if (data.photoDataUrl) {
      try {
        ...
        await doRollback(`enrollment_photos: ${e instanceof Error ? e.message : "unknown error"}`);
      }
    }
...
export const submitAttendance = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [studentRate, ipRate] = await Promise.all([
      ...
      ip
        ? checkRateLimit(`attend:ip:${ip}:${data.sessionId}`, 10, 3600_000)
        : Promise.resolve({ allowed: true, remaining: 10, count: 0 }),
    ]);
    ...
    const sessionAllowlist: string[] =
      session.ip_allowlist && session.ip_allowlist.length > 0 ? session.ip_allowlist : [];
    const defaultCampusCidr = process.env.DEFAULT_CAMPUS_CIDR ?? null;
    const effectiveAllowlist =
      sessionAllowlist.length > 0 ? sessionAllowlist : defaultCampusCidr ? [defaultCampusCidr] : [];
    ...
    const { hasWebauthnExemption, getWebauthnPolicy, decideDeviceGateOutcome } =
      await import("./webauthn.server");
...
export async function requireTeacher(userId: string) {
  ...
  if (!data || data.length === 0) {
    throw new Error("403 Forbidden: Teacher role required");
  }
}
...
// Canonical query for "which subjects does THIS teacher teach".
//
// Root cause this fixes: the `courses.teacher_id` column was the only authority
// for teacher subject assignment. `assignSignupRole` could STEAL a course from
// another teacher (`UPDATE courses SET teacher_id = <new>`) and legacy duplicate
// course codes could multiply rows. The result: a teacher who selected 5 subjects
// could see 7-9, and the previous user's subjects could leak after logout/login.
//
// The `teacher_courses` join table (see migration
// 20260811010000_teacher_course_isolation.sql) is now the canonical source of
// truth. This server function queries it joined to `courses`, filtered
// server-side by the authenticated user's id — never fetching all subjects and
// merging them on the frontend.
export const listMyTeacherSubjects = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    try {
      await requireTeacher(context.userId);
      ...
      // 1. Try canonical teacher_courses join table first
      const { data: tcData, error: tcErr } = await (context.supabase as any)
        .from("teacher_courses")
        .select(
          "course_id, courses(id, code, name, created_at, department_id, semester_id, departments(code, name), semesters(code, name))",
        )
        .eq("teacher_id", context.userId)
      ...
      // 2. Fallback / supplementary: Direct query on courses.teacher_id = context.userId
      const { data: directCourses, error: directErr } = await context.supabase
        .from("courses")
        .select(
          "id, code, name, created_at, department_id, semester_id, departments(code, name), semesters(code, name)",
        )
      ...
      if (!directErr && directCourses) {
        for (const course of directCourses) {
          if (!course?.id) continue;
          if (seen.has(course.id)) continue;
          seen.add(course.id);
          rows.push(course as any);
        }
      }
...
export const listCourseSessions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    try {
      // ACCOUNT-ISOLATION FIX: verify the caller owns this course (or is an
      // admin) before returning its sessions. Previously any authenticated
      // user could list sessions for any course, leaking another teacher's
      // class data.
...
export const createClassSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (course.teacher_id !== context.userId) {
      throw new Error("403 Forbidden: You are not assigned to this subject.");
    }
    ...
    if (error || !row) {
      console.warn(
        "[createClassSession] Primary insert error, retrying with supabaseAdmin:",
        error?.message,
      );
...
export const getOrCreateActiveDemoSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    try {
      ...
      // ACCOUNT-ISOLATION FIX: never auto-create a course with this user's id
      // as teacher_id. Previously any user (including students) could cause a
      // "DEMO-101" course to be created with their user_id as the teacher,
      // polluting the courses table and injecting a fake "subject" into their
      // teacher listing later. Only reuse an existing course owned by this
      // caller (or an admin).
      ...
      if (existingCourse?.id) {
        const isOwner = existingCourse.teacher_id === context.userId;
        if (!isOwner) {
          const { data: adminRow } = await supabaseAdmin
            .from("user_roles")
            .select("role")
            ...
            .eq("role", "admin")
            .maybeSingle();
          if (!adminRow) return { sessionId: "00000000-0000-4000-a000-000000000001" };
        }
        courseId = existingCourse.id;
      }
      // If no DEMO-101 course exists, do NOT create one — only authorized
      // users may proceed, and without a course there is nothing to attach a
      // session to. Fall through to the fail-safe default below.
...
export const submitNfcCheckin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (studentId !== context.userId) {
      ...
      if (!adminRow) {
        return {
          ...
          message:
            "This NFC tag belongs to a different student. You can only check in with your own tag.",
        };
      }
    }
    ...
    const enc = new TextEncoder();
    const tagUidHashBuf = await crypto.subtle.digest(
      "SHA-256",
      enc.encode(data.tagUid) as BufferSource,
    );
...
export const grantSelfTeacherRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    if (!isAdmin) {
      throw new Error(
        "Forbidden: Self-assigning teacher role is restricted. Contact your institution administrator.",
      );
    }
...
export const teacherMarkAttendanceDirect = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if ((sessionData as any).status === "finalized" || (sessionData as any).status === "locked") {
      throw new Error(
        "Attendance for this lecture is finalized and locked. Unlock session to modify.",
      );
    }
    ...
    if (courseData.teacher_id !== context.userId) {
      await supabaseAdmin
        .from("courses")
        .update({ teacher_id: context.userId })
        .eq("id", sessionData.course_id);
    }
    ...
    try {
      ...
      const { data: ledgerEntries } = studentIds.length
        ? await (supabaseAdmin as any)
            .from("attendance_ledger")
            .select(
              "id, student_id, decision, similarity, trust_score, reason_code, gate_reasons, created_at",
            )
            .eq("session_id", data.sessionId)
        : { data: [] };
...
export const searchStudentAttendanceHistory = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ query: z.string().trim().min(1) }).parse(input))
  .handler(async ({ data, context }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    for (const p of profiles) {
      const { data: ledger } = await (supabaseAdmin as any)
        .from("attendance_ledger")
        .select(
          "id, session_id, decision, trust_score, reason_code, created_at, class_sessions(course_id, courses(code, name))",
        )
      ...
      const totalPresent = ((ledger ?? []) as any[]).filter(
        (l) => l.decision === "present" || l.decision === "fallback_present",
      ).length;
      const totalFailed = ((ledger ?? []) as any[]).filter(
        (l) => l.decision === "rejected" || l.decision === "review",
      ).length;
