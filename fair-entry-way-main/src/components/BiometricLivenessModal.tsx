...
async function loadFaceMesh(
  ...
  try {
    ...
    const FaceMesh = mod?.FaceMesh ?? (typeof window !== "undefined" ? (window as any).FaceMesh : null);
    ...
    faceMesh.onResults((results: any) => {
      ...
      const foreheadPts = FOREHEAD_LANDMARK_INDICES
        .map((i) => raw[i])
...
function computeMotionVariance(
...
function assessCameraIntegrity(track: MediaStreamTrack): {
  ...
  try {
    ...
    const virtualKeywords = [
      "obs", "virtual", "manycam", "xsplit",
      "snap camera", "mmhmm", "droidcam", "iriun",
    ];
    ...
    try {
      ...
      if (gl) {
        ...
        if (debugInfo) {
          const renderer = (gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL) as string | null)?.toLowerCase() ?? "";
          const softwareRenderers = ["swiftshader", "llvmpipe", "softpipe", "mesa", "virtualbox", "vmware", "headless"];
...
export const BiometricLivenessModal: React.FC<BiometricLivenessModalProps> = ({
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      if (track) {
        const { suspicionScore, reasons } =
          assessCameraIntegrity(track);
        if (suspicionScore >= 0.5) {
          ...
          setCameraWarning(
            "Limited camera capabilities detected. " +
            "Ensure you are using your built-in webcam.",
          );
        }
      }
  ...
  const collectSample = useCallback(
    (target: "rppg" | "challenge") => {
      ...
      if (target === "rppg") {
        ...
        setRppgProgress(
          Math.min(100,
            Math.round((rppgSamplesRef.current.length / MIN_RPPG_SAMPLES) * 100),
          ),
        );
      } else {
      ...
      if (luminance < 35) {
        setCameraWarning("Low ambient light detected. Face a light source for reliable rPPG pulse detection.");
  ...
  const handleChallengeComplete = useCallback(async () => {
    ...
    const motionVariance =
      metrics.length > 0
        ? metrics.reduce((s, m) => s + m.motionDelta, 0) / metrics.length
        : 0;
    ...
    try {
      ...
      if (result.verdict.decision === "review" || !result.livenessToken) {
        setError(
          "Liveness result requires supervisor review. " +
      ...
      setPhase("success");
      setTimeout(
        () => onSuccess(result.livenessToken!, "rekognition"),
        700,
      );
    } catch (err) {
      setError(
  ...
  const statusText = useMemo((): string => {
    switch (phase) {
      case "idle":       return "Position your face in the frame, then press Verify.";
      case "starting":   return "Starting secure session...";
      case "rppg":       return "Phase 1 of 2: Hold still — measuring blood pulse...";
      case "gap":        return "Preparing color challenge...";
      case "challenge":  return "Phase 2 of 2: Follow the screen colors...";
      case "evaluating": return "Analysing signals on server...";
      case "success":    return "Liveness verified.";
      case "failed":     return "Verification failed. See message below.";
    }
  }, [phase]);
      ...
      >
        ...
        <header className="flex items-center justify-between px-6 py-4
          ...
          >
            ...
            <span className="hidden sm:inline">
              {mediaPipeReady ? "FaceMesh" : "No mesh"}
            </span>
          </div>
        ...
        <div className="px-6 py-4 space-y-3">
            ...
            >
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  >
                    This verification briefly displays rapidly changing
                    coloured lights. If you have photosensitive epilepsy
                    or are sensitive to flashing lights, please use the
          ...
          {cameraWarning && (
            ...
            >
              <AlertTriangle
                className="h-4 w-4 shrink-0 text-amber-600 mt-px"
                aria-hidden="true"
              />
          ...
          >
            ...
            >
              <p
                id={descId}
                className="text-xs font-medium text-center text-slate-200"
              >
                {statusText}
              </p>
            </div>
            ...
            {!streamRef.current && phase !== "failed" && (
              ...
              >
                <Camera
                  className="h-8 w-8 text-indigo-400 animate-pulse"
                  aria-hidden="true"
                />
            ...
            {phase === "rppg" && (
              <div>
                <div className="flex justify-between text-[10px] font-medium
                  ...
                  <label htmlFor={rppgBarId}>
                    rPPG Blood Pulse Sampling
                  </label>
          ...
          {errorMessage && (
            ...
            >
              <AlertTriangle
                className="h-4 w-4 shrink-0 text-red-600 mt-px"
                aria-hidden="true"
              />
          ...
          <p className="text-center text-[10px] leading-relaxed
            ...
            Liveness signals are evaluated server-side. This system
            provides strong protection against photo and replay attacks.
            It cannot guarantee detection of all attack types without
