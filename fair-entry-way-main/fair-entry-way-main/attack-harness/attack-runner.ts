...
export function assertGate(
  ...
  if (opts.expectedDecision) {
    if (r.decision === opts.expectedDecision) {
      ...
      notes.push(`decision expected="${opts.expectedDecision}" got="${r.decision ?? "(none)"}" ✗`);
      passed = false;
    }
  }
  ...
  if (opts.expectedFailedGate && r.gateReasons) {
    ...
    if (gate && gate.ok === false) {
      ...
      notes.push(
        `gate[${opts.expectedFailedGate}] expected ok=false but got: ${JSON.stringify(gate)} ✗`,
      );
      passed = false;
    }
  }
...
export function printResult(label: string, result: AttendanceResult) {
  ...
  console.log(
    `    gateReasons: ${JSON.stringify(result.gateReasons, null, 6).replace(/^/gm, "    ")}`,
  );
}
