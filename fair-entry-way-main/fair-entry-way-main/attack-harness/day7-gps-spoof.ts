...
async function main() {
  ...
  if (spatialA?.distance_m) {
    info(
      `  Server-calculated distance: ${spatialA.distance_m.toFixed(1)}m (radius: ${config.classroom.radiusM}m)`,
    );
  }
  ...
  if (spatialD?.distance_m) {
    info(
      `  Control distance: ${spatialD.distance_m.toFixed(1)}m (radius: ${config.classroom.radiusM}m)`,
    );
  }
