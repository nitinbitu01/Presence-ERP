...
test.describe("Accessibility Toolbar & i18n Locale Switching E2E", () => {
  ...
  test("toggles High Contrast mode and applies CSS class to root html element", async ({
    page,
  }) => {
    const contrastBtn = page.locator(
      "button:has-text('Contrast'), button[aria-label*='High Contrast']",
    );
  ...
  test("switches application language between 5 locales without full page reload", async ({
    page,
  }) => {
