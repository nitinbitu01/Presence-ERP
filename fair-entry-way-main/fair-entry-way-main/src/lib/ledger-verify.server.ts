...
export const verifyLedgerChain = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: rows, error } = await supabaseAdmin
      ...
      .select(
        "id, session_id, student_id, decision, similarity, trust_score, trust_breakdown, record_hash, previous_entry_id, created_at, profiles:student_id(display_name, roll_no)",
      )
    ...
    return {
      rows: results,
      allVerified: results.every((r: any) => r.verified),
