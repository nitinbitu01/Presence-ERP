...
const LIVENESS_HMAC_KEY_RAW = (() => {
  ...
  console.warn(
    "[Liveness][DEV] Using ephemeral HMAC key. " + "Sessions will not survive process restart.",
  );
  return ephemeral;
})();
...
export type LivenessActionStep = "blink" | "turn_left" | "turn_right" | "nod" | "smile";
...
async function getRekognitionSdk(): Promise<RekognitionSDK> {
  ...
  const mod: any = await import(/* @vite-ignore */ "@aws-sdk/client-rekognition").catch(() => {
    throw new PresenceErpError(
      "INTERNAL_ERROR",
...
function classifyAwsError(err: unknown): PresenceErpError {
  ...
  const map: Record<string, [string, string]> = {
    ThrottlingException: [
      "RATE_LIMITED",
      "Liveness service temporarily busy. Please retry in a moment.",
    ],
    ...
    SessionNotFoundException: [
      "NOT_FOUND",
      "Liveness session not found or expired. Please start a new session.",
    ],
  ...
  const [code, message] = map[name] ?? [
    "INTERNAL_ERROR",
    "Liveness service error. Please try again.",
  ];

  if (name === "AccessDeniedException") {
    console.error(
      "[Liveness][AWS] Access denied — check IAM policy for rekognition:CreateFaceLivenessSession.",
    );
  } else if (!map[name]) {
...
async function hmacVerify(payload: string, sigHex: string): Promise<boolean> {
  ...
  return crypto.subtle.verify("HMAC", key, sigBytes.buffer as ArrayBuffer, ENCODER.encode(payload));
}
 ...
 */
async function computeSessionBindingHash(userId: string, vendorSessionId: string): Promise<string> {
...
export async function verifyFido2UserVerification(
  ...
  try {
    ...
    const base64 = authenticatorDataBase64Url
      ...
      .replace(/_/g, "/")
      .padEnd(
        authenticatorDataBase64Url.length + ((4 - (authenticatorDataBase64Url.length % 4)) % 4),
        "=",
      );
...
export const verifyLivenessSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<LivenessSessionResult> => {
    ...
    try {
      ...
      throw new PresenceErpError("INTERNAL_ERROR", "Corrupted liveness session record.");
    }
    ...
    const recomputedBinding = await computeSessionBindingHash(context.userId, vendorId);
    const storedSig = await hmacSign(`verify-binding:${record.bindingHash}`);
    const bindingMatch = await hmacVerify(`verify-binding:${recomputedBinding}`, storedSig);
    ...
    if (!bindingMatch || record.userId !== context.userId) {
      ...
      throw new PresenceErpError("UNAUTHORIZED", "Session binding validation failed.");
    }

    // ── Challenge Verification ────────────────────────────────────────────
    if (data.completedSteps?.length) {
      const storedChallengeSig = await redisGetDel(`${NS_CHALLENGE}${vendorId}:${context.userId}`);
      if (!storedChallengeSig) {
        ...
        throw new PresenceErpError("UNAUTHORIZED", "Action challenge not found or already used.");
      }
    ...
    const { token, expiresAt } = await issueLivenessToken(context.userId, vendorId, record.method);
...
export async function assertLiveness(
  ...
  throw new PresenceErpError("FORBIDDEN", result.reason ?? "Liveness assertion failed.");
}
...
export function analyzeFacialDepthMap(landmarks3D: FacialPoint3D[]): {
  ...
  const pts = (landmarks3D ?? []).filter((p) => p != null && isFinite(p.z));
...
export async function computeReferenceFrameSha256(
  ...
  const bytes = typeof frameInput === "string" ? ENCODER.encode(frameInput) : frameInput;
...
export async function detectImpossibleTravel(
  ...
  const prevLat: number | undefined = last.server_latitude ?? (last.metadata as any)?.latitude;
  const prevLon: number | undefined = last.server_longitude ?? (last.metadata as any)?.longitude;
  ...
  const a =
    Math.sin(dLat / 2) ** 2 +
    Math.cos(toRad(prevLat)) * Math.cos(toRad(currentLat)) * Math.sin(dLon / 2) ** 2;

  const distanceKm = EARTH_RADIUS_KM * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
  const timeDeltaMinutes = (Date.now() - new Date(last.created_at).getTime()) / 60_000;
  const isSuspicious =
    distanceKm > IMPOSSIBLE_TRAVEL_KM && timeDeltaMinutes < IMPOSSIBLE_TRAVEL_MINUTES;
...
export async function detectDescriptorReuse(
  ...
  if (!descriptor.length || descriptor.length > 512) {
    throw new PresenceErpError("VALIDATION_FAILED", "Face descriptor must be 1–512 dimensions.");
  }
  ...
  try {
    const { data, error } = await (supabaseAdmin as any).rpc("find_similar_face_descriptor", {
 ...
 */
function goertzel(signal: number[], targetHz: number, sampleRate: number): number {
...
export function analyzeRppgPulse(rppgSamples: SkinColorSample[]): {
  ...
  const samples = [...rppgSamples].sort((a, b) => a.timestampMs - b.timestampMs);
  const n = samples.length;

  // Estimate actual sample rate from timestamps
  const durationMs = samples[n - 1]!.timestampMs - samples[0]!.timestampMs;
  const sampleRate = durationMs > 0 ? (n * 1_000) / durationMs : 30;
  ...
  for (let hz = RPPG_BPM_MIN / 60; hz <= RPPG_BPM_MAX / 60; hz += freqStepHz) {
  ...
  const noisePower =
    results
      ...
      .reduce((s, r) => s + r.power, 0) / Math.max(1, results.length - 3);
  ...
  const validBpmRange = estimatedBpm >= RPPG_BPM_MIN && estimatedBpm <= RPPG_BPM_MAX;
  ...
  return {
    ...
    details: [
      ...
      `Sample rate: ${sampleRate.toFixed(1)} fps, ` + `window: ${(durationMs / 1_000).toFixed(1)}s`,
      pulseDetected
...
export function verifyChallengeColorReflections(
  ...
  const matchScore = totalEvaluated > 0 ? matches / totalEvaluated : 0.0;
...
export function evaluateMultiModalLiveness(
  ...
  if (rppg.pulseDetected) {
    ...
    reasons.push(`rPPG SNR ${rppg.snr.toFixed(2)} is borderline — routing to review.`);
  } else {
    rppgScore = 0.2;
    flags.push("NO_BLOOD_PULSE");
    reasons.push("No physiological blood pulse detected in facial skin ROIs.");
  }
  ...
  if (entropy.suspicious) {
    ...
    reasons.push(entropy.reason ?? "Suspicious frame entropy detected.");
  }
  ...
  if (input.motionVariance < 0.001 && entropy.suspicious) {
    ...
    reasons.push("Zero motion variance + low entropy — virtual camera injection suspected.");
    writeAuditLog({
  ...
  if (moire.isDigitalScreen) {
    ...
    reasons.push(`Screen Moiré pattern detected (confidence: ${moire.moireConfidence}%).`);
  }

  // Combined motion + entropy score
  const motionRaw = Math.min(1, Math.max(0, input.motionVariance / 0.05));
  const motionScore = (entropy.score * 0.6 + motionRaw * 0.4) * (moire.isDigitalScreen ? 0.3 : 1.0);
  ...
  if (!depth.is3DFace) {
    reasons.push(
      depth.reason ?? "Low pseudo-depth variance (auxiliary signal only — see documentation).",
    );
  }
  ...
  const overallScore =
    Math.round(
      (rppgScore * 0.35 + challengeScore * 0.35 + motionScore * 0.2 + depthScore * 0.1) * 100,
    ) / 100;
  ...
  if (overallScore >= 0.65 && criticalCount === 0) {
    decision = "live";
  } else if (overallScore < 0.4 || criticalCount >= 2) {
    decision = "spoof";
  } else {
    decision = "review";
  }
  ...
  return {
    ...
    reasons,
    flags,
    weights: { rppg: 0.35, challenge: 0.35, motion: 0.2, depth: 0.1 },
  };
}
...
export const verifyMultiModalFaceLiveness = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) =>
    z
      ...
      .refine((d) => d.challengePhaseStartMs >= d.rppgPhaseEndMs, {
        message: "challengePhaseStartMs must be ≥ rppgPhaseEndMs — phases must not overlap.",
  ...
  .handler(
    async ({
      ...
      const rppgContaminated = data.rppgSamples.some((s) => s.timestampMs > data.rppgPhaseEndMs);
      ...
      if (verdict.decision === "spoof") {
        ...
        throw new PresenceErpError(
          ...
          `Liveness check failed: ${
            verdict.reasons.find((r) => r.length < 120) ?? "Anti-spoof verification failed."
          }`,
        );
      }
      ...
      const sessionRef = `mm_${Date.now()}_${Array.from(crypto.getRandomValues(new Uint8Array(4)))
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("")}`;
