...
import { fastInjectCheck, semanticInjectCheck } from "@/lib/presence-ai/injection-guard";
import { trimToContextWindow } from "@/lib/presence-ai/token-utils";
import { retrieveRelevantRecords } from "@/lib/presence-ai/rag";
import { buildSystemPrompt } from "@/lib/presence-ai/prompt-builder";
...
function smartAnswer(question: string, records: AttendanceRecord[], profile: any): string {
  ...
  const getAbsenceReason = (r: AttendanceRecord) => {
    ...
    return (
      reasonMap[code] ??
      `The system recorded reason code: "${code}". Contact your instructor for details.`
    );
  };
  ...
  if (
    is("attendance", "rate", "percentage", "percent", "how much", "overall") ||
    is("my record", "my status")
  ) {
    if (totalValid === 0) {
    ...
    return [
      ...
      `**Eligibility Goals:**`,
      n75 === 0
        ? "✅ 75% target — Already achieved!"
        : `• To reach 75%: attend **${n75} more consecutive sessions**`,
      n85 === 0
        ? "✅ 85% target — Already achieved!"
        : `• To reach 85%: attend **${n85} more consecutive sessions**`,
      "",
      latestRecord
  ...
  if (
    ...
    return [
      ...
      n75 === 0
        ? "✅ Already achieved — keep it up!"
        : `→ Attend **${n75} consecutive sessions** without missing any.`,
      "",
      "**To Reach 85% (Distinction Level):**",
      n85 === 0
        ? "✅ Already achieved — excellent!"
        : `→ Attend **${n85} consecutive sessions** without missing any.`,
      "",
      rate < 75
  ...
  if (is("last session", "latest", "most recent", "yesterday", "today")) {
    ...
    const gates = (latestRecord.trust_breakdown?.components ?? [])
      .map(
        (c) =>
          `  • **${c.label}**: ${(c.achieved * 100).toFixed(0)}% — ${c.achieved >= (c.threshold ?? 0.7) ? "✅ PASS" : "❌ FAIL"}`,
      )
      .join("\n");
    return [
...
export const askPresence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const systemPrompt = [forecastBlock, memoryBlock, baseSystemPrompt]
        .filter(Boolean)
        .join("\n\n");
      ...
      if (!validation.safe) {
        console.warn("[Presence AI] Output validation issues:", validation.issues);
      ...
      await track({
        ...
        success: !streamError,
        errorType: streamError ? (streamError as Error).message : undefined,
      });

      // ── STEP 15: Update Memory (async — never blocks response) ────
      // Fires after we return — student gets their answer immediately
      import("@/lib/presence-ai/memory")
        .then((m) => m.updateMemoryAsync(userId, data.question, finalAnswer, memory, apiKey))
        .catch(() => {});
