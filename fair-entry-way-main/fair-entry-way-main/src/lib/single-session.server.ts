...
export const registerUserActiveSession = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) => z.object({ sessionId: z.string().min(8) }).parse(input))
...
export const validateUserActiveSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: userRes, error } = await supabaseAdmin.auth.admin.getUserById(context.userId);
