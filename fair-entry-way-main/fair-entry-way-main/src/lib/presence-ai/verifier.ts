...
export function verifyResponse(aiText: string, forecast: AttendanceForecast): VerificationResult {
  ...
  const overallConfidence =
    claims.length > 0 ? claims.reduce((sum, c) => sum + c.confidence, 0) / claims.length : 0.9; // no detectable claims → assume high confidence (no numbers to verify)
...
function extractAndVerifyPercentages(text: string, forecast: AttendanceForecast): VerifiedClaim[] {
  const claims: VerifiedClaim[] = [];
