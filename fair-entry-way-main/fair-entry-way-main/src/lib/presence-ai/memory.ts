...
export async function updateMemoryAsync(
  ...
  updateMemoryImpl(userId, question, answer, currentMemory, apiKey).catch((err) =>
...
function buildRuleBasedSummary(memory: StudentMemory): string {
  ...
  parts.push(
    `Preferred language: ${memory.preferredLanguage}. ${memory.interactionCount} total interactions.`,
  );
  return parts.join(" ");
}
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      messages: [
        {
          role: "system",
          content:
            "Compress the student's attendance assistant history into 2-3 sentences. Focus on: recurring issues, unresolved problems, what they need help with. Max 100 words.",
        },
    ...
    signal:
      typeof AbortSignal !== "undefined" && typeof (AbortSignal as any).timeout === "function"
        ? (AbortSignal as any).timeout(5_000)
        : undefined,
  });
...
export function formatMemoryForPrompt(memory: StudentMemory): string {
  ...
  if (memory.preferredLanguage !== "en") {
    ...
    parts.push(
      `Preferred language: ${langNames[memory.preferredLanguage] ?? memory.preferredLanguage}.`,
    );
  }
