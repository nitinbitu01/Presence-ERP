...
function makeKey(userId: string, question: string): string {
  ...
  const hash = crypto.createHash("sha256").update(`${userId}::${normalized}`).digest("hex");
...
export async function getCached(userId: string, question: string): Promise<CachedResponse | null> {
