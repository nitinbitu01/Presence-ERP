...
export type AlertType = "session_starting_soon" | "daily_warning_digest" | "critical_threshold";
...
export async function generateAlertsForStudent(userId: string): Promise<AttendanceAlert[]> {
  const forecast = await computeForecast(userId);
  const alerts: AttendanceAlert[] = [];
  const now = new Date();

  // Alert 1: session starting soon
  const imminent = forecast.upcomingSessions.filter(
    (s) => s.minutesUntil <= 60 && s.minutesUntil >= 0,
  );
...
export async function persistAlertsForUser(
  ...
  try {
    ...
    for (const alert of alerts) {
      await db.from("ai_alerts").upsert(
        {
          id: alert.id,
