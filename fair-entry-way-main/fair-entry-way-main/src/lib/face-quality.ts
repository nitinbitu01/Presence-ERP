...
export function isEmbeddingEligible(pose: { yaw: number; pitch: number; roll?: number }): boolean {
  ...
  return (
    Math.abs(pose.yaw) <= EMBEDDING_MAX_YAW_DEG && Math.abs(pose.pitch) <= EMBEDDING_MAX_PITCH_DEG
  );
}
...
export const QUALITY_REASON_MESSAGES: Record<string, string> = {
  ...
  blown_out:
    "Lighting is too harsh or you are facing a bright light source — try facing a window or softer light.",
};
...
function laplacianVariance(imageData: ImageData): number {
  ...
  for (let y = 1; y < height - 1; y++) {
    for (let x = 1; x < width - 1; x++) {
      const idx = y * width + x;
      const lap =
        grey[idx - width] + grey[idx + width] + grey[idx - 1] + grey[idx + 1] - 4 * grey[idx];
      responses.push(lap);
    }
  }
  ...
  const variance = responses.reduce((a, v) => a + (v - mean) ** 2, 0) / responses.length;
  return variance;
}
...
export function assessFrameQuality(
  ...
  checksTotal++;
  const interEyeDist = landmarks.length >= 46 ? dist2d(landmarks[36], landmarks[45]) : 0;
