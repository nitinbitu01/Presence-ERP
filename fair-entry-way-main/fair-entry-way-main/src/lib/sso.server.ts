...
export type SsoProviderType =
  "azure_ad" | "shibboleth" | "okta" | "google_workspace" | "custom_saml";
...
async function nonceSet(state: string, meta: NonceMeta): Promise<void> {
  try {
    ...
    await redis.setex(`${NONCE_KEY_PREFIX}${state}`, NONCE_TTL_SEC, JSON.stringify(meta));
  } catch (err) {
...
async function sessionCreate(token: string, session: SsoSession): Promise<void> {
  try {
    const redis = await getRedis();
    await redis.setex(`${SESSION_KEY_PREFIX}${token}`, SESSION_TTL_SEC, JSON.stringify(session));
  } catch (err) {
 ...
 */
async function getSamlInstance(provider: SsoProviderConfig, callbackUrl: string): Promise<any> {
  ...
  if (!provider.ssoUrl || !provider.entityId || !provider.certificatePem) {
    throw new PresenceErpError("INTERNAL_ERROR", "SAML provider configuration is incomplete.");
  }
...
export function buildSamlAuthnRequest(
  ...
  if (!["https:", "http:"].includes(acsUrl.protocol)) {
    throw new PresenceErpError("VALIDATION_FAILED", "assertionConsumerUrl must use HTTPS or HTTP.");
  }
...
async function exchangeOidcCode(
  ...
  try {
    ...
    console.warn(
      `[SSO][OIDC] Userinfo fetch failed for provider ${provider.id}, using id_token claims only.`,
    );
  }
...
export function verifyDomainBoundary(email: string, provider: SsoProviderConfig): void {
  ...
  const allowed = provider.domains.some((d) => d.normalize("NFKC").toLowerCase() === emailDomain);
...
export const initiateSsoLogin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<SsoAuthRequestResult> => {
    ...
    const appBase =
      data.redirectUrl ?? process.env.APP_BASE_URL ?? "https://rru-presence.pages.dev";
    ...
    if (provider.protocol === "oidc") {
      ...
      if (!certCheck.valid) {
        console.error(
          `[SSO] Certificate validation failed for provider ${provider.id}: ${certCheck.reason}`,
        );
        throw new PresenceErpError("INTERNAL_ERROR", "SSO provider configuration error.");
      }
  ...
  .handler(
    async ({
      ...
      if (provider.protocol === "saml2" && data.samlResponse) {
        ...
        rawAttributes = await exchangeOidcCode(provider, data.code, data.codeVerifier, callbackUrl);
      } else {
        throw new PresenceErpError(
          "VALIDATION_FAILED",
...
export const validateSsoSession = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({ data }): Promise<{ valid: boolean; user?: SsoUserIdentity; reason?: string }> => {
      const request = getRequest();
      const ipAddress = extractClientIp(request);
      const userAgent = extractUserAgent(request);
...
export const configureSsoProvider = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) =>
    z
      .object({
        ...
        groupRoleMapping: z.record(z.string(), z.enum(["student", "teacher", "admin"])).optional(),
        attributeMapping: z
          .object({
  ...
  .handler(async ({ data, context }): Promise<{ success: boolean; providerId: string }> => {
    const request = getRequest();
    const ipAddress = extractClientIp(request);
    const userAgent = extractUserAgent(request);
