  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSupabaseSession) {
        ...
        setSuccess(
          "Password updated successfully! All sessions have been logged out. Redirecting to sign in...",
        );
      } else {
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-12 bg-background">
      <div className="w-full max-w-md rounded-2xl border border-border bg-card/90 backdrop-blur-md p-8 shadow-2xl">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          />
          <div>
            <div className="font-bold text-foreground text-lg leading-snug">Presence ERP</div>
        ...
        <p className="text-xs text-muted-foreground mb-6">
          Create a strong, unique password for your ERP account. Your password will be securely
          encrypted.
        </p>
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-5">
              ...
              <div className="text-xs flex items-center gap-1.5 -mt-2">
                {pwnedState.checking ? (
                  ...
                  <span className="text-destructive font-medium flex items-center gap-1">
                    ...
                    ⚠️ Found in {pwnedState.count.toLocaleString()} data breaches (unsafe - choose
                    another)
                  </span>
                ) : (
                  <span className="text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1">
                    <ShieldCheck className="h-3.5 w-3.5 shrink-0" />✓ Password not found in known
                    data breaches
                  </span>
                )}
              </div>
            ...
            <div className="rounded-lg border border-border/50 bg-muted/30 p-3 text-xs text-muted-foreground">
              ...
              <p className="leading-relaxed">
                After updating your password, all active sessions will be terminated for security.
