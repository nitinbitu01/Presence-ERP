...
import { clearUserSessionState } from "@/lib/session-cleanup";
import {
  AlertCircle,
  CheckCircle2,
  ChevronRight,
...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    if (reason === "concurrent_login") {
      setError(
        "Signed out because your account was accessed from another device. Please sign in again.",
      );
    }
  ...
  const handleSignUpSubmit = async (e?: React.FormEvent) => {
    ...
    if (pwStrength.score < 3) {
      setError(
        "Password is too weak. Must include uppercase, lowercase, numbers, and special characters.",
      );
      return;
    }
  ...
  const handleOAuth = async (provider: "google" | "azure") => {
    ...
    try {
      ...
      if (oauthErr) {
        setError(
          `OAuth Error: ${oauthErr.message}. Ensure '${provider}' is enabled in your Supabase Dashboard -> Authentication -> Providers.`,
        );
        setBusy(false);
      }
    } catch (err) {
        ...
        <div className="hidden lg:flex w-[62%] relative overflow-hidden bg-gradient-to-br from-[#020617] via-[#0c1a3a] to-[#0d1b4b] flex-col justify-between p-12 text-white">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <Link to="/" className="flex items-center gap-3 group">
              <img
                ...
                className="w-10 h-10 object-contain rounded-xl bg-white p-0.5 shadow-sm transition-transform group-hover:scale-105"
              />
              <div className="flex flex-col">
                <span className="text-base font-extrabold tracking-tight leading-none text-white">
                  Presence ERP
                </span>
                <span className="text-[10px] font-semibold text-white/40 uppercase tracking-widest mt-1">
                  Attendance Integrity Platform
                </span>
              </div>
            </Link>
          ...
          <div className="relative z-10 max-w-xl my-auto py-8">
            ...
            <p className="text-base text-white/50 leading-relaxed mb-8 max-w-md font-normal">
              Every sign-in is bound to your device, geofenced to your campus, and cryptographically
              signed. No proxies. No shared passwords.
            </p>
                ...
                <div className="relative w-20 h-20 flex-shrink-0">
                  <svg className="w-20 h-20 -rotate-90" viewBox="0 0 88 88">
                    <circle
                      ...
                      cy="44"
                      r="39"
                      className="fill-none stroke-white/10 stroke-[5.5]"
                    />
                    <circle
                      cx="44"
                      cy="44"
                      ...
                      strokeDashoffset={
                        RING_CIRCUMFERENCE - (RING_CIRCUMFERENCE * trustScore) / 100
                      }
                    />
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                    ...
                    <span className="text-[8.5px] font-semibold text-white/40 uppercase">
                      / 100
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  },
                  {
                    icon: "📍",
                    name: "Geofence",
                    sub: "Campus boundary · 50m radius",
                    val: "Valid",
                    check: true,
                  },
                  {
                  ...
                  <div
                    ...
                    style={{ animation: `signal-appear 0.4s ${0.5 + idx * 0.1}s ease forwards` }}
                  >
                    ...
                    <div className="flex items-center gap-1.5 font-bold text-white/90 text-xs">
                      <div className="w-3.5 h-3.5 rounded-full bg-emerald-500 flex items-center justify-center text-[8px] text-white font-bold">
                        ✓
                      </div>
                      {s.val}
                    </div>
                ...
                {signInStep === "sso" && (
                  <div>
                    ...
                    <h2 className="text-2xl font-extrabold text-slate-900 mb-2">
                      Continue with {ssoDomain}
                    </h2>
                    <p className="text-xs text-slate-500 mb-5">
                      You will be redirected to your institution's secure single sign-on portal.
                    </p>

                    <div className="p-3.5 rounded-xl bg-slate-100 border border-slate-200 flex items-center gap-3 mb-5">
                      <div className="w-9 h-9 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-base">
                        🏛
                      </div>
                      <div>
                        <div className="text-xs font-semibold text-slate-900">{email}</div>
                        <div className="text-[10.5px] text-slate-500">
                          Verified via institutional SSO
                        </div>
                      </div>
                    </div>
            ...
            {mode === "signup" && (
              <div>
                ...
                <div className="flex items-center gap-2 mb-6">
                  <div
                    className={`h-1 rounded-full flex-1 transition-all ${signUpStep >= 1 ? "bg-slate-900" : "bg-slate-200"}`}
                  ></div>
                  <div
                    className={`h-1 rounded-full flex-1 transition-all ${signUpStep >= 2 ? "bg-slate-900" : "bg-slate-200"}`}
                  ></div>
                  <div
                    className={`h-1 rounded-full flex-1 transition-all ${signUpStep >= 3 ? "bg-slate-900" : "bg-slate-200"}`}
                  ></div>
                </div>
                      ...
                      {[
                        {
                          id: "student",
                          title: "Student",
                          desc: "Mark attendance in your enrolled classes",
                          icon: "🎓",
                        },
                        {
                          id: "teacher",
                          title: "Faculty",
                          desc: "Create sessions and review attendance analytics",
                          icon: "👤",
                        },
                        {
                          id: "admin",
                          title: "Administrator",
                          desc: "Manage institution, users, and compliance",
                          icon: "🏛",
                        },
                      ].map((r) => (
                        <button
                        ...
                        >
                          <div
                            className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg ${selectedRole === r.id ? "bg-slate-900 text-white" : "bg-slate-100"}`}
                          >
                            {r.icon}
                          </div>
                          ...
                          <ChevronRight
                            className={`w-4 h-4 ${selectedRole === r.id ? "text-slate-900" : "text-slate-300"}`}
                          />
                        </button>
                      ))}
                    </div>
                ...
                {signUpStep === 2 && (
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mb-5">
                      We'll use these to verify your institutional identity
                    </p>
                      ...
                      {(selectedRole === "student" || selectedRole === "teacher") && (
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 mb-2">
                            Select your subjects to display active class sessions & live face
                            check-in on student dashboard
                          </p>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        ...
                                        setSelectedSubjects(
                                          selectedSubjects.filter((s) => s !== subLabel),
                                        );
                                      }
                                    }}
                ...
                {signUpStep === 3 && (
                  <form onSubmit={handleSignUpSubmit}>
                    ...
                    <p className="text-xs text-slate-500 mb-5">
                      Biometric enrolment will follow after sign-up
                    </p>
                            ...
                            <div className="flex gap-1 mb-1">
                              {[1, 2, 3, 4].map((bar) => (
                                <div
                                  ...
                                  style={{
                                    backgroundColor:
                                      bar <= pwStrength.score ? pwStrength.color : "#e2e8f0",
                                  }}
                                ></div>
                              ))}
                            </div>
