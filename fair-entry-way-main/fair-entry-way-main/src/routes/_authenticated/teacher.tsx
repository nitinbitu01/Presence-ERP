...
import {
  ...
  reviewFallbackRequest,
  refreshSessionOtp,
  teacherMarkAttendanceDirect,
  finalizeClassSession,
  getClassSessionRosterStatus,
  searchStudentAttendanceHistory,
  grantSelfTeacherRole,
...
import {
  ...
  exportCourseRegisterCsv,
} from "@/lib/admin.functions";
import {
  createExam,
  listExamsForCourse,
  updateExam,
  listEnrolledStudentsForMarksEntry,
  bulkEnterMarks,
...
export const Route = createFileRoute("/_authenticated/teacher")({
  ...
  validateSearch: (s: Record<string, unknown>) => ({
    tab: (["live", "exceptions", "leaves", "history", "search", "timetable", "exams"].includes(
      s.tab as string,
    )
      ? s.tab
      : "live") as TabType,
...
function useShortcut(map: Record<string, () => void>) {
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      const key = `${mod ? "mod+" : ""}${e.altKey ? "alt+" : ""}${e.shiftKey ? "shift+" : ""}${e.key.toLowerCase()}`;
      if (ref.current[key]) {
        e.preventDefault();
        ref.current[key]();
      }
...
> = {
  emerald: {
    wrap: "border-emerald-500/20 from-emerald-500/5 to-emerald-600/10",
    text: "text-emerald-600 dark:text-emerald-400",
    icon: "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400",
    glow: "bg-emerald-400",
  },
  blue: {
    wrap: "border-blue-500/20 from-blue-500/5 to-blue-600/10",
    text: "text-blue-600 dark:text-blue-400",
    icon: "bg-blue-500/15 text-blue-600 dark:text-blue-400",
    glow: "bg-blue-400",
  },
  violet: {
    wrap: "border-violet-500/20 from-violet-500/5 to-violet-600/10",
    text: "text-violet-600 dark:text-violet-400",
    icon: "bg-violet-500/15 text-violet-600 dark:text-violet-400",
    glow: "bg-violet-400",
  },
  rose: {
    wrap: "border-rose-500/20 from-rose-500/5 to-rose-600/10",
    text: "text-rose-600 dark:text-rose-400",
    icon: "bg-rose-500/15 text-rose-600 dark:text-rose-400",
    glow: "bg-rose-400",
  },
  amber: {
    wrap: "border-amber-500/20 from-amber-500/5 to-amber-600/10",
    text: "text-amber-600 dark:text-amber-400",
    icon: "bg-amber-500/15 text-amber-600 dark:text-amber-400",
    glow: "bg-amber-400",
  },
  slate: {
    wrap: "border-border from-muted/20 to-muted/40",
    ...
    >
      <div className="flex items-start justify-between">
        <div className="space-y-1.5 flex-1 min-w-0">
          ...
          <AnimatedNumber
            value={value}
            className={cn("text-3xl font-black tabular-nums block", s.text)}
          />
          {sub && <p className="text-[11px] text-muted-foreground truncate">{sub}</p>}
        </div>
        <span
          className={cn("flex h-10 w-10 items-center justify-center rounded-xl shrink-0", s.icon)}
          aria-hidden
        >
      ...
      <div
        className={cn(
          "pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full blur-2xl opacity-20",
          s.glow,
        )}
        aria-hidden
      />
...
const ProgressRing = memo(function ProgressRing({
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`${displayPct}% attendance`}
    >
      ...
      <div className="relative z-10 space-y-5">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-0.5">
            ...
            <h2 className="mt-1.5 text-sm font-black text-white/90">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {[
            ...
            >
              ...
              {shortcut && (
                <kbd
                  className="ml-0.5 rounded border border-white/10 bg-white/5 px-1 py-0.5 text-[9px] font-mono text-white/40"
                  aria-hidden
                >
                  {shortcut}
                </kbd>
...
const AttendanceRingPanel = memo(function AttendanceRingPanel({
  counts,
}: {
  counts: RosterCounts;
}) {
  ...
  return (
    ...
    >
      ...
      {counts.expected > 0 && (
        <div
          className="flex h-2 w-full overflow-hidden rounded-full gap-px"
          role="img"
            ...
            .map((d) => (
              <motion.div
                ...
                style={{
                  ...
                  width: `${(d.val / counts.expected) * 100}%`,
...
const STATUS_CONFIG = {
  ...
  failed: {
    cls: "bg-rose-500/10   border-rose-500/20   text-rose-600   dark:text-rose-400",
    Icon: XCircle,
    ...
    <div ref={parentRef} className="overflow-auto" style={{ height: "400px" }}>
      <div className="sticky top-0 z-10 grid grid-cols-[1fr_100px_120px_1fr_90px] gap-0 border-b border-border bg-muted/40 text-[11px] font-bold text-muted-foreground">
        {["Student", "Roll No", "Status", "Detail", "Action"].map((h) => (
          <div key={h} className={cn("px-4 py-3", h === "Action" && "text-right")}>
            {h}
          </div>
        ))}
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5 px-4 py-3 min-w-0">
                <div
                  className="h-7 w-7 shrink-0 rounded-lg bg-primary/10 border border-primary/10 flex items-center justify-center text-[11px] font-black text-primary"
                  aria-hidden
                >
              ...
              <div className="px-4 py-3 font-mono text-[11px] text-muted-foreground truncate">
                {row.rollNo}
              </div>
              ...
              <div
                className="px-4 py-3 text-[11px] text-muted-foreground truncate"
    ...
    <>
      <Toaster
        ...
        toastOptions={{
          classNames: {
            toast:
              "rounded-xl border border-border bg-card text-foreground shadow-xl text-xs font-medium",
            title: "font-bold",
...
function TeacherDashboard() {
  ...
  const listCoursesFn = useServerFn(listMyTeacherSubjects);
  ...
  const [rosterCounts, setRosterCounts] = useState<RosterCounts>({
    expected: 0,
    present: 0,
    verifying: 0,
    failed: 0,
  ...
  const refreshCourses = useCallback(async () => {
    ...
    setCourses(rows);
    setSelected((prev) => {
      if (prev && rows.some((c) => c.id === prev.id)) {
        return prev;
      }
  ...
  const loadRoster = useCallback(
    async (sid: string, silent = false) => {
      ...
      try {
        ...
        setRosterCounts(
          res.counts ?? { expected: 0, present: 0, verifying: 0, failed: 0, notAttempted: 0 },
        );
  ...
  useEffect(() => {
    if (!activeSessionId) {
      setRosterData([]);
      setRosterCounts({ expected: 0, present: 0, verifying: 0, failed: 0, notAttempted: 0 });
      return;
    }
  ...
  useShortcut(
    useMemo(
      () => ({
        ...
        "alt+r": () => {
          if (activeSessionId) handleGenerateOtp(activeSessionId);
        },
        "alt+p": () => setProjectingCode(true),
        "alt+ ": () => setPaused((p) => !p),
        "alt+f": () => {
          if (activeSessionId) handleFinalizeSession(activeSessionId);
        },
        "alt+1": () => setTab("live"),
  ...
  const handleInstantActivate = async () => {
    ...
    try {
      ...
      setIsTeacher(true);
      await refreshCourses();
      toast.success("Faculty access activated!", {
        description: "Welcome to the Teacher Command Center.",
      });
    } catch (e) {
  ...
  const handleRequestRole = async (e: React.FormEvent) => {
    ...
    if (roleReason.trim().length < 15) {
      toast.warning("Reason too short", { description: "Please provide at least 15 characters." });
      return;
    }
    setBusy(true);
    try {
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    if (!geoVal.valid) {
      toast.error("Invalid geofence", { description: geoVal.error });
      return;
    }
    const ipVal = validateIpAllowlist("");
    if (!ipVal.valid) {
      toast.error("Invalid IP list", { description: ipVal.error });
      return;
    }
    setBusy(true);
    const tid = toast.loading("Starting lecture…");
    try {
      const res = (await addSessionFn({
        data: {
          ...
          geoLat: geoVal.lat!,
          geoLng: geoVal.lng!,
          radiusM: geoVal.radius!,
          ipAllowlist: ipVal.ips,
        },
      })) as { id: string };
  ...
  const handleManualMark = useCallback(
    async (sid: string, studentId: string, reasonCode: string, note: string) => {
        ...
        p.map((r) =>
          r.studentId === studentId
            ? {
                ...
                verificationDetail: `Manual: ${note || reasonCode}`,
              }
            : r,
        ),
      );
      ...
      try {
        await directMarkFn({
          data: { sessionId: sid, studentId, reasonCode: reasonCode as any, reasonNote: note },
        });
        ...
        toast.error("Mark failed", {
          ...
          action: {
            label: "Retry",
            onClick: () => handleManualMark(sid, studentId, reasonCode, note),
          },
        });
      }
    },
  ...
  const handleExport = useCallback(
    async (courseId: string) => {
      ...
      try {
        const { filename, csv } = (await exportCsvFn({ data: { courseId } })) as {
          filename: string;
          csv: string;
        };
  ...
  const filteredRoster = useMemo(() => {
    let r =
      rosterFilter === "all" ? rosterData : rosterData.filter((x) => x.status === rosterFilter);
    if (rosterSearch.trim()) {
      const q = rosterSearch.toLowerCase();
      r = r.filter(
        (x) => x.displayName.toLowerCase().includes(q) || x.rollNo?.toLowerCase().includes(q),
      );
    }
    return r;
    ...
    return (
      <div className="mx-auto max-w-lg px-6 py-20">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="relative overflow-hidden rounded-3xl border border-destructive/20 bg-card p-8 shadow-2xl text-center space-y-6"
        >
          <div
            className="pointer-events-none absolute inset-0 bg-gradient-to-br from-destructive/5 to-rose-500/5"
            aria-hidden
          />
          <motion.div
            animate={{ scale: [1, 1.05, 1] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
            className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-destructive/20 bg-destructive/10"
          >
            <Lock className="h-8 w-8 text-destructive" />
          </motion.div>
          <div>
            <h1 className="text-xl font-black text-foreground">Faculty Access Restricted</h1>
            <p className="mt-2 text-xs text-muted-foreground max-w-xs mx-auto leading-relaxed">
              You are logged in as a Student. Access to the Teacher Command Center is restricted to
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="w-full rounded-2xl bg-primary py-3.5 text-sm font-black text-primary-foreground shadow-xl flex items-center justify-center gap-2.5"
          >
  ...
  return (
    <LazyMotion features={domAnimation} strict>
      ...
      <div className={cn("mx-auto max-w-7xl px-4 pb-16 pt-6 space-y-5", !online && "mt-8")}>
        ...
        >
          <div
            className="pointer-events-none absolute right-0 top-0 h-full w-2/5 bg-gradient-to-l from-primary/5 to-transparent"
            aria-hidden
          />
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <h1 className="text-xl font-black tracking-tight text-foreground">
                    Presence ERP — Faculty Dashboard
                  </h1>
                </div>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                Command
                <kbd
                  className="rounded border border-border bg-muted px-1 py-0.5 text-[9px] font-mono text-muted-foreground"
                  aria-hidden
                >
                  ⌘K
                </kbd>
        ...
        >
          {courses.length > 0 ? (
            ...
            <div className="flex-1 text-center py-2">
              <p className="text-sm font-bold text-amber-600 dark:text-amber-500 flex items-center justify-center gap-2">
                <AlertTriangle className="h-4 w-4" />
                No Subjects Assigned
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                You did not select any subjects during signup. Please contact administration to
              ...
              >
                <Icon
                  className={cn(
                    ...
                    active && key === "live" && activeSessionId && "text-emerald-500",
                  )}
                  aria-hidden
                />
                {label}
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === "live" && (
            ...
            >
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <StatCard
                  label="Present"
                  value={rosterCounts.present}
                  icon={CheckCircle2}
                  color="emerald"
                  index={0}
                  sub={`of ${rosterCounts.expected}`}
                />
                <StatCard
                  label="Verifying"
                  value={rosterCounts.verifying}
                  icon={Activity}
                  color="violet"
                  index={1}
                />
                ...
                <StatCard
                  label="Not Attempted"
                  value={rosterCounts.notAttempted}
                  icon={HelpCircle}
                  color="slate"
                  index={3}
                />
                        ...
                        >
                          <div className="text-center space-y-2.5">
                            ...
                            <p className="text-xs text-muted-foreground max-w-sm mx-auto leading-relaxed">
                              Configure geofence and start attendance for{" "}
                          ...
                          >
                            <fieldset>
                              <legend className="text-xs font-bold text-foreground mb-3">
                                Geofence
                              </legend>
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Students who encountered verification failures during lectures.
                </p>
              </div>
              <ErrorBoundary>
                <ReviewQueueSection />
          ...
          {tab === "leaves" && (
            ...
            >
              <div>
                <h2 className="text-base font-black text-foreground">
                  OD & Leave Request Approvals
                </h2>
                <p className="text-xs text-muted-foreground">
                  Requests where students specifically designated you as the authorizing faculty
                  member.
                </p>
              </div>
              <ErrorBoundary>
                    ...
                    <tr>
                      {["Date & Time", "Status", "Geofence", "Action"].map((h) => (
                        <th
                          key={h}
                          scope="col"
                          className={cn("px-5 py-3.5 font-bold", h === "Action" && "text-right")}
                        >
                          {h}
                        </th>
                      ))}
          ...
          {tab === "timetable" && selected && (
            ...
            >
              <ErrorBoundary>
                <TimetableSection course={selected} />
              </ErrorBoundary>
            </m.div>
          )}
                ...
                >
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                      <div
                        className="h-11 w-11 rounded-xl border border-primary/20 bg-primary/10 flex items-center justify-center font-black text-primary"
                        aria-hidden
                      >
                  ...
                  <div className="h-2 w-full overflow-hidden rounded-full bg-border">
                    <m.div
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min(r.overallPct, 100)}%` }}
                      ...
                      role="img"
                      aria-label={`${r.overallPct}% attendance`}
                    />
                  </div>
                  <p
                    className={cn(
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {r.recentRecords.map((rec: any) => (
                          <div
                            key={rec.id}
                            className="flex items-center justify-between rounded-xl border border-border bg-muted/30 px-3.5 py-2"
                          >
                            ...
                            <span
                              className={cn(
                                "ml-3 shrink-0 rounded-full border px-2.5 py-0.5 text-[10px] font-black",
...
const ReviewQueueSection = memo(function ReviewQueueSection() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setRows((await listFn()) as TeacherReviewRow[]);
    } catch (e) {
      toast.error("Queue load failed", { description: (e as Error).message });
    } finally {
      setLoading(false);
    }
  }, [listFn]);
  ...
  if (!rows.length)
    return (
      ...
      >
        ...
        <p className="text-xs text-muted-foreground mt-1">
          No borderline check-ins awaiting review.
        </p>
      </m.div>
    );
            ...
            >
              <div className="min-w-0 space-y-0.5">
                ...
                <p className="text-[11px] text-muted-foreground">
                  ...
                  <span
                    className={cn(
                      "font-black",
                      (r.similarity || 0) > 0.8 ? "text-amber-600" : "text-rose-600",
                    )}
                  >
              ...
              <m.button
                ...
                onClick={() =>
                  setActive({
                    id: r.id,
                    studentName: `Student ${r.student_id.slice(0, 8)}`,
                    type: "review",
...
const FallbackQueueSection = memo(function FallbackQueueSection() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setRequests((await listFn()) as FallbackRequestRow[]);
    } catch (e) {
...
const TeacherLeaveRequestsSection = memo(function TeacherLeaveRequestsSection() {
  ...
  const handleAction = async (id: string, action: "approved" | "rejected") => {
    ...
    const tid = toast.loading(
      action === "approved" ? "Approving OD/Leave request…" : "Rejecting request…",
    );
    try {
      await reviewFn({ data: { requestId: id, action } });
      toast.success(action === "approved" ? "Approved OD/Leave request!" : "Rejected request", {
        id: tid,
      });
      load();
    } catch (e) {
...
const TimetableSection = memo(function TimetableSection({ course }: { course: Course }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setEntries((await listTt({ data: { courseId: course.id } })) as TimetableEntry[]);
    } catch (e) {
      toast.error("Load failed", { description: (e as Error).message });
    } finally {
      setLoading(false);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.error("Generation failed", { id: tid, description: (e as Error).message });
    }
  };
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Define recurring slots and batch-generate sessions.
        </p>
      </div>
              ...
              <tr>
                {["Day", "Timing", "Room", "Action"].map((h) => (
                  <th
                    key={h}
                    scope="col"
                    className={cn("px-5 py-3 font-bold", h === "Action" && "text-right")}
                  >
                    {h}
                  </th>
                ))}
...
const ExamsSection = memo(function ExamsSection({ course }: { course: Course }) {
  ...
  const loadExams = useCallback(async () => {
    setExLoading(true);
    try {
      const res = (await listExamsFn({
        data: { courseId: course.id },
      })) as unknown as TeacherExamRow[];
      setExams(res);
      if (res.length && !activeId) setActiveId(res[0].id);
    } catch (e) {
      toast.error("Exams load failed", { description: (e as Error).message });
    } finally {
      setExLoading(false);
    }
  ...
  useEffect(() => {
    ...
    listStudFn({ data: { examId: activeId } })
      .then((res) => {
        ...
        for (const s of list)
          m[s.student_id] = {
            marks: s.obtained_marks != null ? String(s.obtained_marks) : "",
            absent: Boolean(s.is_absent),
            remarks: s.remarks || "",
          };
        setMarksState(m);
      })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const row = (await createExamFn({
        data: {
          courseId: course.id,
          title: title.trim(),
          examType: examType as any,
          maxMarks: Number(maxMarks),
          weightagePct: Number(weightage),
          examDate: undefined,
        },
      })) as TeacherExamRow;
      setTitle("");
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast.error("Publish failed", { id: tid, description: (e as Error).message });
    }
  };
  ...
  const stats = useMemo(() => {
    const graded = students.filter((s) => {
      const st = marksState[s.student_id];
      return st && !st.absent && st.marks !== "";
    });
    const avg = graded.length
      ? graded.reduce((s, x) => s + Number(marksState[x.student_id].marks), 0) / graded.length
      : 0;
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter(
      (s) => s.display_name.toLowerCase().includes(q) || s.roll_no?.toLowerCase().includes(q),
    );
        ...
        {showCreate && (
          ...
          >
            ...
            <form
              ...
              className="grid grid-cols-2 gap-3 sm:grid-cols-6 items-end"
            >
              ...
              {!exams.length ? (
                ...
                exams.map((ex, i) => (
                  <m.div
                    ...
                    role="listitem"
                    aria-current={activeId === ex.id ? "true" : undefined}
                    className={cn(
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-black text-foreground truncate flex-1">
                        {ex.title}
                      </span>
                      <span
                        className={cn(
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {EXAM_TYPE_LABELS[ex.exam_type]} · {ex.max_marks}M · {ex.weightage_pct}%
                    </p>
                  </m.div>
                ))
              )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {EXAM_TYPE_LABELS[activeExam.exam_type]} · Max: {activeExam.max_marks}M ·
                    Weight: {activeExam.weightage_pct}%
                  </p>
                </div>
                <div className="flex items-center gap-2">
              ...
              <>
                <div className="grid grid-cols-[1fr_90px_110px_60px_1fr_60px] border-b border-border bg-muted/10 text-[11px] font-bold text-muted-foreground">
                  {["Student", "Roll", "Marks", "Absent", "Remarks", "Grade"].map((h, i) => (
                    <div key={h} className={cn("px-4 py-3", i === 5 && "text-right")}>
                      {h}
                    </div>
                  ))}
                </div>
                <div ref={tbodyRef} style={{ height: "400px", overflow: "auto" }}>
                  <div style={{ height: `${rowVirt.getTotalSize()}px`, position: "relative" }}>
                    {rowVirt.getVirtualItems().map((vRow) => {
                      ...
                      const s = marksState[st.student_id] || {
                        marks: "",
                        absent: false,
                        remarks: "",
                      };
                      ...
                      const pct =
                        mn !== null && !s.absent
                          ? Math.round((mn / activeExam.max_marks) * 100)
                          : null;
                        ...
                        >
                          ...
                          <div className="px-4 py-3">
                            <input
                              ...
                              disabled={s.absent}
                              placeholder={`0–${activeExam.max_marks}`}
                              value={s.marks}
                          ...
                          <div className="px-4 py-3 flex justify-center">
                            <input
                              type="checkbox"
                              checked={s.absent}
                              onChange={(e) => upd({ absent: e.target.checked })}
