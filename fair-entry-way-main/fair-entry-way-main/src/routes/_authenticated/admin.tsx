...
import { bindNfcTag, unbindNfcTag, listNfcBindings } from "@/lib/nfc-provisioning.server";
...
function AdminPage() {
  ...
  const adminTabItems = [
    { id: "health", label: "System Health" },
    {
      id: "role_requests",
      label: "Role Requests",
      count: roleRequests.filter((r) => r.status === "pending").length,
    },
          ...
          {tab === "health" && (
            <div className="mt-8 rounded-lg border border-destructive/40 bg-destructive/5 p-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                This will permanently delete <strong>all non-admin users</strong> from Auth,
                profiles, roles, face embeddings, and attendance records. The account{" "}
              ...
              {purgeResult && (
                <div className="mt-3 rounded-md bg-emerald-500/10 border border-emerald-500/30 p-3 text-xs text-emerald-800 dark:text-emerald-200">
                  ...
                  {purgeResult.deletedEmbeddings} embeddings, {purgeResult.deletedAttendance}{" "}
                  attendance records. Admin <code>{purgeResult.preservedAdminEmail}</code>{" "}
                  preserved.
                </div>
              )}
              {purgeError && (
...
function NfcProvisioningPane() {
  ...
  const handleUnbind = async (studentId: string) => {
    if (
      !window.confirm("Unbind this NFC tag? The student will need a new tag to check in via NFC.")
    )
      return;
    setBusy(true);
    setErr(null);
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-5 shadow-sm mt-6">
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-4">
        ...
        {resultMsg && (
          <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">{resultMsg}</p>
        )}
...
function WebAuthnHardwarePolicyToggle() {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-5 shadow-sm mt-6">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Control whether hardware key binding (TouchID, YubiKey, FaceID) is mandatory for attendance
        check-ins.
      </p>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {(["mandatory", "recommended", "optional"] as const).map((p) => (
          ...
          >
            {p === "mandatory"
              ? "🔒 Mandatory (Strict Zero-Trust)"
              : p === "recommended"
                ? "⚠️ Recommended (Grace Period)"
                : "⚪ Optional (Legacy)"}
          </button>
        ))}
      </div>
      ...
      {msg && (
        <p className="mt-3 text-xs font-medium text-emerald-600 dark:text-emerald-400">{msg}</p>
      )}
    </div>
  );
}
      ...
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Automated incident runbooks, disaster recovery snapshot checks, multi-region failover,
            key rotation, and push telemetry.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {keyRes && (
            <div className="rounded bg-background p-2 text-[10px] font-mono space-y-1">
              <p className="text-emerald-600 font-bold">
                ✓ Keys Rotated: {keyRes.rotatedKeyTypes?.join(", ")}
              </p>
        ...
        <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-3">
          ...
          {telemRes && (
            <div className="rounded bg-background p-2 text-[10px] font-mono space-y-1">
              <p className="text-emerald-600 font-bold">
                ✓ Pushed {telemRes.exportedMetricsCount} metrics to {telemRes.exporterTarget}
              </p>
            </div>
          )}
        </div>
...
function ComplianceGrievancePane() {
  ...
  const [delReason, setDelReason] = useState(
    "Student right-to-erasure request under DPDP Act 2023",
  );
  const [delMsg, setDelMsg] = useState<string | null>(null);

  const [sancStudentId, setSancStudentId] = useState("");
  const [penaltyType, setPenaltyType] = useState<
    "warning" | "attendance_deduction" | "suspension" | "exam_disqualification"
  >("warning");
  const [sancReason, setSancReason] = useState(
    "Attempted attendance proxy fraud using non-enrolled device",
  );
    ...
    <div className="rounded-lg border border-border bg-card p-5 shadow-sm mt-6 space-y-6">
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Full account erasure, minor parental consent governance, Grievance Officer escalation,
            and disciplinary penalty sanctions.
          </p>
        </div>
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        {result && (
          <div className="mt-3 rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-800">
            ...
            {result.invited} invited, {result.updated} profiles updated, {result.enrolled} enrolled
            into course.
...
function BiometricRetentionPane() {
  ...
  return (
    <div className="mt-4 rounded-lg border border-border bg-card p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        This also runs automatically once a day (pg_cron, 03:00 UTC) on projects with the pg_cron
        extension enabled — the button below is the manual/fallback trigger for self-hosted or
        Cloudflare-only deployments where that isn't available, and to run it on demand.
      </p>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-lg border border-border bg-muted/20 p-4">
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">
            ...
            status. Not auto-erased — there's no reliable way to tell a long-tenured active student
            from one who left without formal offboarding, so these need a human look.
          </p>
                    ...
                    <div className="w-full bg-secondary/50 rounded-t h-full flex items-end overflow-hidden max-w-[48px]">
                      <div
                        className={`w-full rounded-t transition-all duration-500 ${
                          d.attendancePct >= 75 ? "bg-emerald-500" : "bg-destructive"
                        }`}
                  ...
                  <div className="pt-2 flex items-baseline justify-between">
                    <span
                      className={`text-lg font-bold ${
                        d.attendancePct >= 75 ? "text-emerald-500" : "text-destructive"
                      }`}
                    >
                      {d.attendancePct}%
                    </span>
      ...
      <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Test all 7 anti-proxy security gates with 1-click live simulation. Events are logged
              instantly to the Security Audit Feed and trigger active webhooks.
            </p>
          </div>
