...
export function LedgerChainViewer({
  rows,
  allVerified,
  onAttemptTamper,
  tamperResult,
}: LedgerChainViewerProps) {
  return (
    <div className="w-full max-w-3xl mx-auto py-8">
      {/* Integrity Summary Banner */}
      <div
        className={`mb-8 p-4 rounded-xl border flex items-center gap-3 font-bold shadow-sm ${allVerified ? "bg-emerald-50 border-emerald-200 text-emerald-800 dark:bg-emerald-950/30 dark:border-emerald-800 dark:text-emerald-300" : "bg-red-50 border-red-200 text-red-800 dark:bg-red-950/30 dark:border-red-800 dark:text-red-300"}`}
      >
        ...
        {rows.map((row, idx) => {
          ...
          const decisionColors =
            row.decision === "present"
              ...
              : row.decision === "review"
                ? "bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-500/20 dark:text-amber-300"
          ...
          return (
            <div key={row.id} className="relative w-full flex flex-col items-center group">
              ...
              <div
                className={`w-1 h-8 ${isVerified ? "bg-emerald-400 dark:bg-emerald-600" : "border-l-2 border-dashed border-red-500"}`}
              ></div>

              {/* Node Card */}
              <div
                className={`w-full relative bg-card border ${isVerified ? "border-border shadow-sm" : "border-red-400 shadow-red-500/20 shadow-lg"} rounded-2xl p-5 transition-all hover:shadow-md z-10`}
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border ${decisionColors}`}
                      >
                        {row.decision}
                      </span>
                      {row.trustScore !== null && (
