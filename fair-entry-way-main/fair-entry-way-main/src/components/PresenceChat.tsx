...
function StreamingCursor() {
  return <span className="inline-block w-2 h-4 ml-0.5 bg-current animate-pulse rounded-sm" />;
}
...
function ForecastBanner({
  ...
  forecast: NonNullable<PresenceStreamMetadata["forecast"]>;
}) {
...
function VerificationBadge({
  ...
  badge: NonNullable<PresenceStreamMetadata["verificationBadge"]>;
}) {
  ...
  return (
    <span
      className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${styles[badge.variant] ?? styles.inferred}`}
    >
      {badge.label}
    </span>
  );
      ...
      >
        ...
        {!isUser && meta && !message.isStreaming && (
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            ...
            {meta.verificationBadge && <VerificationBadge badge={meta.verificationBadge} />}
          </div>
        )}
...
export function PresenceChat() {
  ...
  const { messages, isStreaming, isLoading, rateLimitRemaining, sendMessage, clearHistory, abort } =
    usePresenceStream();
  ...
  const suggestions =
    riskLevel === "critical" || riskLevel === "failed"
      ? [
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2">
          {rateLimitRemaining != null && (
            <span className="text-xs text-gray-400">{rateLimitRemaining} queries left</span>
          )}
          {messages.length > 0 && (
            <button
      ...
      {latestForecast && !isStreaming && <ForecastBanner forecast={latestForecast} />}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <h3 className="text-gray-800 font-semibold mb-1">Ask about your attendance</h3>
            <p className="text-gray-500 text-sm mb-6">
              I know your records, remember your past issues, and can predict your future
              trajectory.
            </p>
