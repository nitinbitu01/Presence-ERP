...
export function checkPasswordStrength(password: string, userEmail?: string) {
  const hasCommonPattern = COMMON_PATTERNS.some((pat) => password.toLowerCase().includes(pat));
      ...
      {showStrength && !validation && value.length > 0 && (
        <div className="space-y-2 rounded-lg border border-border/50 bg-muted/30 p-3 text-xs">
          ...
          <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
            <div
              className={`h-full transition-all duration-300 ${strength.barWidth.split(" ")[1]}`}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 pt-1 text-[11px]">
            <div
              className={`flex items-center gap-1 ${strength.checks.length ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-muted-foreground"}`}
            >
            ...
            <div
              className={`flex items-center gap-1 ${strength.checks.uppercase ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-muted-foreground"}`}
            >
            ...
            <div
              className={`flex items-center gap-1 ${strength.checks.lowercase ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-muted-foreground"}`}
            >
            ...
            <div
              className={`flex items-center gap-1 ${strength.checks.number ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-muted-foreground"}`}
            >
            ...
            <div
              className={`flex items-center gap-1 ${strength.checks.special ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-muted-foreground"}`}
            >
            ...
            <div
              className={`flex items-center gap-1 ${strength.checks.noCommonPattern ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-destructive font-medium"}`}
            >
              {strength.checks.noCommonPattern ? (
                <Check className="h-3 w-3" />
              ) : (
                <X className="h-3 w-3" />
              )}
            ...
            {userEmail && (
              <div
                className={`col-span-2 flex items-center gap-1 ${strength.checks.noUserInfo ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-destructive font-medium"}`}
              >
                {strength.checks.noUserInfo ? (
                  <Check className="h-3 w-3" />
                ) : (
                  <X className="h-3 w-3" />
                )}
