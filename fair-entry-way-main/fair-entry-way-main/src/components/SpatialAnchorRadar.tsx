...
export function SpatialAnchorRadar({ signals, className = "" }: SpatialAnchorRadarProps) {
  if (!signals) {
    return (
      <div
        className={`rounded-xl border border-border bg-card p-4 text-xs text-muted-foreground animate-pulse ${className}`}
      >
  ...
  const { gps, mockLocation, motionPresence, network, deviceFingerprint, confidenceScore } =
    signals;
  const isMockRiskHigh = mockLocation.risk === "high" || mockLocation.risk === "medium";

  return (
    <div
      className={`rounded-xl border border-indigo-500/30 bg-slate-950 p-4 text-white shadow-lg space-y-3 ${className}`}
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
        ...
        <div className="rounded-lg bg-slate-900 border border-slate-800 p-2.5 space-y-1">
          ...
          <p className="text-[10px] font-mono text-slate-400 truncate">
            {signals.hmac
              ? `${signals.hmac.slice(0, 12)}…`
              : deviceFingerprint.canvasSha256.slice(0, 12)}
          </p>
        </div>
      </div>
