...
import {
  getEnrolledProfileSummary,
  getOrCreateActiveDemoSession,
...
export function AppNav() {
  ...
  // Track session user ID & handle auth state changes
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === "SIGNED_OUT" || (sessionUserId && newUserId !== sessionUserId)) {
  ...
  const { data: userRoles } = useQuery({
    queryKey: ["my-user-roles", sessionUserId],
    queryFn: () => getRolesFn(),
    staleTime: 5000,
  ...
  const { data: profileSummary } = useQuery({
    queryKey: ["enrolled-profile-summary", sessionUserId],
  ...
  // ACCOUNT-ISOLATION FIX: clear all user-specific state (React Query cache,
  // localStorage session keys, sessionStorage) before signing out so the next
  // account never sees the previous user's profile name, subjects, or data.
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2.5 sm:px-6">
        <div className="flex items-center gap-3">
          ...
          <nav className="ml-4 flex items-center gap-1 sm:gap-2">
            ...
            {userRoles?.isStudent && !userRoles?.isTeacher && !userRoles?.isAdmin && (
              <>
                <Link
                  to="/student"
                  className="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium text-muted-foreground hover:bg-accent hover:text-foreground [&.active]:bg-primary/10 [&.active]:text-primary"
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      const targetSession =
                        res?.sessionId || "00000000-0000-4000-a000-000000000001";
