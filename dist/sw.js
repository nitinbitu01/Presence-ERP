...
const CACHE_VERSION = "presence-erp-v5-20260812";
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open(STATIC_CACHE).then((cache) => cache.addAll(["/", "/favicon.ico", "/rru-logo.png"])),
  );
  self.skipWaiting();
});
...
function isModelManifest(url) {
  return url.pathname.includes("/models/") && url.pathname.endsWith(".json");
}
